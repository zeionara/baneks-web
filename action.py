...
def get_response_content(request, action_function):
    if request.args and 'id' in request.args:
        return json.dumps(action_function(request.args.get('id'), request.remote_addr))
    else:
        return json.dumps({'status': db_adapter.STATUS_FAILURE, 'message': 'Not found arg `id` in request'})
